...
binary.model <- function(psi, id, xidep) {
  ...
  logit <- beta0 + beta_1 * ((Feature_1 - 2 * Feature_2)^2) + beta_2 * Feature_3/pmax((1 + Feature_4)^2, 0.1) + beta_3 * Feature_5
...
saemix_model <- saemixModel(
  ...
  description = "DeepGLMM Rep. Nonlinear binary outcome model",
  psi0 = matrix(c(0, 0, 0, 0), ncol = 4, byrow = TRUE, 
                dimnames = list(NULL, c("beta0", "beta1", "beta2", "beta3"))), 
  transform.par = c(0, 0, 0, 0),
  covariance.model = diag(c(1,0,0,0)), omega.init=diag(c(0.1,0,0,0))
)


# Step 5: Fit the Model
fit <- saemix(saemix_model, saemix_data, list(
  nbiter.saemix = c(600, 100),            # Number of iterations
...
logit <- pmin(estimates_re$beta0 + 
  estimates_re$beta1 * ((test_data$Feature_1 - 2 * test_data$Feature_2)^2) +
  estimates_re$beta2 * test_data$Feature_3 / (1 + test_data$Feature_4)^2 +
  estimates_re$beta3 * test_data$Feature_5, 10)
