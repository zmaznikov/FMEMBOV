...
path <- "nonlin-3_var0.1_s12345.csv"
...
binary.model <- function(psi, id, xidep) {
  ...
  logit <- beta0 + beta_1 * log(1 + Feature_1) + beta_2 * (Feature_2^2)/exp(Feature_3) + beta_3 * (Feature_4^3)/ Feature_5
...
logit <- estimates_re$beta0 + estimates_re$beta1 * log(1+test_data$Feature_1) +
  estimates_re$beta2 * (test_data$Feature_2^2)/exp(test_data$Feature_3) +
