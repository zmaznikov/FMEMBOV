...
for i = 1:I
    % Training data: t = 1 to 14
    y_train0{i} = y0{i}(1:14);
    
    % Validation data: t = 15 to 17
    y_val0{i} = y0{i}(15:17);
    
    % Test data: t = 18 to 20
    y_test0{i} = y0{i}(18:20);
end
...
nn = [5,5];
mdl0 = deepGLMMfit(X_train0,y_train0,...  
                  X_val0,y_val0,...
...
rng(1)
Pred1 = deepGLMMpredict(mdl0,X_test0); 
...
% Preallocate a vector to hold all responses
response_deep = []; % This will store all y values from the cell array

% Combine all responses from the y cell array
for i = 1:numel(y0) % Loop over each individual
    response_deep = [response_deep; y0{i}]; % Append the 20x1 response vector to the list
end
...
% Add the response column to the all_data table
all_data.Response_Deep = response_deep;
...
for i = 1:20000
    rng(i)
    Results = table(predict(glme, glmm_test_data), random('Binomial', 1, predict(glme, glmm_test_data)), glmm_test_data.Response_Deep);
    % Results
...
Results = table(predict(tree, glmm_test_data(:, predictorColumns)), glmm_test_data.Response_Deep);
...
y_train_deepglm = table2array(all_data(all_data.Time <= 17, {'Response_Deep'}));

X_test_deepglm = table2array(all_data(all_data.Time > 17, selected_columns));
y_test_deepglm = table2array(all_data(all_data.Time > 17, {'Response_Deep'}));
