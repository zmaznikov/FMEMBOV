...
for i = 1:I
    ...
    features = [ones(size(individual_data, 1), 1), individual_data{:, 1:num_features}];
...
% Preallocate storage
b = normrnd(1, 1, [I, 1]); % Random effects b_i ~ N(1, std)
...
for i = 1:I
    ...
    X_i = X0{i}; % X{i} is a 20x11 matrix (intercept + 10 features)
    
    % Generate epsilon_{it} ~ N(0, 1) for individual i
    epsilon{i} = normrnd(0, 1, [T, 1]);
    
    % Compute a_{it} using the given formula
    x = X_i(:, 2:end); % Features only (excluding intercept)
    
    % Formula for a_{it}
    sum_x = sum(x, 2); % Sum of features across each row
    a{i} = sum_x .* cos(sum_x) ...
           + 2 * x(:, 1) .* x(:, 2) ...
           + b(i) + epsilon{i};
...
'Response ~ Feature_1 + Feature_2 + Feature_3 + Feature_4 + Feature_5 + Feature_6 + Feature_7 + Feature_8 + Feature_9 + Feature_10',...
...
%% DeepGLM data prep
selected_columns = {'Feature_1', 'Feature_2', 'Feature_3', 'Feature_4', 'Feature_5', 'Feature_6', 'Feature_7', 'Feature_8', 'Feature_9', 'Feature_10'};
...
predictorColumns = {'Feature_1', 'Feature_2', 'Feature_3', 'Feature_4', 'Feature_5', 'Feature_6', 'Feature_7', 'Feature_8', 'Feature_9', 'Feature_10'}; 
...
%%
writetable(all_data, '../Data/lmmnn_var10_s12345.csv');
