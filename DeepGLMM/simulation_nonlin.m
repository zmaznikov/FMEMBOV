...
%%
plotResiduals(FEglme,'histogram','ResidualType','Pearson')
%% 
plotResiduals(glme,'fitted','ResidualType','Pearson')

%%
plotResiduals(FEglme,'fitted','ResidualType','Pearson')
%% 
plotResiduals(glme,'lagged','ResidualType','Pearson')

%%
plotResiduals(FEglme,'lagged','ResidualType','Pearson')
...
for i = 1:20000
    ...
    % Make prediction on a test set with true response
    yProb = predict(mdlLR,X_test_deepglm);
    MCR_LR = [MCR_LR; mean(abs(y_test_deepglm - random('Binomial', 1, yProb)))];
end
...
%%
writetable(all_data, '../Data/nonlin-1_var10_s12345.csv');